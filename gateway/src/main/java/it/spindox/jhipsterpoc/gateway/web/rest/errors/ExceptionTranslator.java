...
public class ExceptionTranslator extends ResponseEntityExceptionHandler implements ExceptionTranslation {
    ...
    private ProblemDetailWithCause getProblemDetailWithCause(Throwable ex) {
        if (
            ex instanceof it.spindox.jhipsterpoc.gateway.service.UsernameAlreadyUsedException
        ) return (ProblemDetailWithCause) new LoginAlreadyUsedException().getBody();
        if (
            ex instanceof it.spindox.jhipsterpoc.gateway.service.EmailAlreadyUsedException
        ) return (ProblemDetailWithCause) new EmailAlreadyUsedException().getBody();
        if (
            ex instanceof it.spindox.jhipsterpoc.gateway.service.InvalidPasswordException
    ...
    private boolean containsPackageName(String message) {
        ...
        return StringUtils.containsAny(
            ...
            "io.",
            "de.",
            "it.spindox.jhipsterpoc.gateway"
        );
    }
}
